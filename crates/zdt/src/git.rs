...
//! saved. Not on every keystroke: `git diff` reads the index and the object store, and doing that
//! between two characters would be a process spawn per keystroke for a gutter mark that is only
//! ever read at rest.
//!
//! The marks themselves go into the editor's `"git"` decoration layer, beside the diagnostics'
//! `"lsp"` one — which is the whole reason layers are named.
