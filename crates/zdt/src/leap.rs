...
//!
//! Not the keymap. Once a leap has started every key is either a character it is aiming at or a
//! label, so a keymap that answered any of them would make some letters unreachable. The modal
//! layer asks here first and only falls through when nothing is in progress.
//!
//! # Operator-pending
//!
//! `ds{ab}` deletes to where the leap lands, so a leap started while an operator is waiting has to
//! come back as a motion rather than as a jump. That is why choosing answers the byte rather than
//! moving anything: the caller knows whether an operator is pending and this does not.
...
impl Leaping {
    ...
    pub fn key(
        ...
        // Anything that is not a character — an arrow, a modifier chord, `<Esc>` — ends it. A
        // leap is three keystrokes; anything else means the hands have moved on.
        ...
        match leap.phase {
            ...
            Phase::Choosing(_) => {
                ...
                match chosen {
                    ...
                    // A key that labels nothing ends it rather than being ignored: a leap that
                    // stayed open through a mistyped label would eat the keys after it too.
                    None => Took::Cancelled,
                }
            }
        }
