...
//! owns them as a value and writes them itself. This application's own properties go through a
//! sheet installed under one name: the editor's colours, the syntax captures, and the terminal's
//! palette. Replacing that sheet keeps its place in the cascade, so switching a theme cannot
//! silently change what beats what.
//!
//! Both are built from the same file, so somebody writing a theme writes one block of
//! declarations. The token schema ignores the properties it has never heard of, and the cascade
//! ignores nothing.
...
///
/// One name for every theme. Installing under a name that is already there replaces the text and
/// leaves the sheet where it is, so a theme switch is a repaint.
...
/// Writes the settings that are style into the cascade.
///
/// Fonts and sizes are CSS, so they belong in a sheet. Threading them through every component that
/// draws text would put the same value in fifty places.
/// The size everything in the interface is drawn at, when nothing says otherwise.
///
/// The interface's own sheets are written in terms of `--zdt-ui-size`, so the size setting
/// reaches every rule. A rule that says `12px` ignores the setting, so there should be none. This
/// is the fallback for a document with no settings installed yet.
pub const DEFAULT_UI_SIZE: f32 = 12.0;
#[must_use]
pub fn settings_sheet(config: &zdt_core::Config) -> String {
    // Two derived sizes, and no ladder. The interface has one size, and a smaller one for the
    // things that annotate it: a heading over a group, a count beside a name. Everything else
    // takes the first.
...
pub fn ZdtTheme(
    ...
    // A guard, and not a bare name. The sheet's content is state and has to go when this
...
///
/// Replacing under the same name keeps its place in the cascade, so editing it while the editor
/// runs is a repaint.
...
pub fn fallback() -> ThemeSource {
    zdt_core::builtin_theme("oldworld").unwrap_or_else(|| {
        // Unreachable while `oldworld` is compiled in. If it ever is not, this answers an empty
        // theme. An unstyled window is still a window the user can quit.
...
mod tests {
    ...
    fn a_pinned_scheme_writes_no_media_query() {
        // Pinned, so nothing in the sheet may change under the interface when the desktop's
        // setting does. That is the whole difference between pinning and deferring.
