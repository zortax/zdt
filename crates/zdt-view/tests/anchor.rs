...
//!
//! What is asserted: a surface is solved by the solver, and stays placed across its own surface
//! being rebound and taken away.
//!
//! These do *not* reproduce the disposal that motivated them. The testkit's frame leaves scopes
//! standing where a real window tears them down, so they pass against the unfixed `place` too. The
...
impl Fixture {
    ...
    /// Whether the solver placed it. The fallback places it roughly.
...
fn a_placement_made_against_a_bound_surface_keeps_its_viewport() {
    ...
    // acquired on the effect's first run. Then something re-runs that effect.
    let fixture = Fixture::open();
    fixture.mount_a_surface();
...
fn a_placement_is_solved_rather_than_guessed() {
    // The fallback in `place` is deliberately invisible, because a surface it could not solve is
    // still put somewhere. So a placement that quietly stopped solving would look exactly like one
    // that worked. This asserts that the solver is what the tests above exercise.
    let fixture = Fixture::open();
    fixture.mount_a_surface();
...
/// the placement belongs to a component's scope.
#[zgui::component]
fn Anchored(
