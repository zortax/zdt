//! The leap labels.
//!
//! One absolutely-positioned character per place the leap could go, put where the editor says that
//! byte is on screen. Nothing about the editor changes: the labels are ordinary elements over the
//! top of it, which is the whole reason `point_for_byte` was worth adding.
//!
//! Drawn inside the pane rather than over the window, so a label is clipped by the window it
...
pub fn LeapLabels(
    ...
    // change, which is once per leap rather than once per frame.
    let placed = {
