...
//! against what `git` itself says. What is asserted here is the layer above them: that the panel
//! reads what the repository holds, that the caret moves between its lists, and that `s` on a file
//! is the file git then reports as staged.
//!
//! Every one of these builds a real repository by running `git`, because a fixture built with the
//! same library under test could agree with it and both be wrong.

use std::path::{Path, PathBuf};
use std::process::Command;

use zdt::gitui::{GitUi, List, Selected, View};
...
///
/// The panel reads on a worker, so every assertion has to let the work finish — which `settle`
/// does by advancing both clocks.
struct Panel {
    window: Window,
    git: GitUi,
}
...
impl Panel {
    fn open(temp: &Temp) -> Self {
        ...
        let git = window.scope.with(|| {
            let workspace = Workspace::new(Project::at(temp.root()));
            GitUi::new(workspace)
        });
...
fn a_project_that_is_not_in_one_says_so_rather_than_failing() {
    ...
    // A temporary directory can itself be inside somebody's repository, so this asserts only that
    // asking is safe — never that the answer is no.
...
fn the_caret_walks_the_list_it_is_in_and_stops_at_the_ends() {
    // Clamped rather than wrapped: these are lists somebody reads in order, and a `j` at the
    // bottom that jumps to the top is a `j` that loses their place.
...
/// A vim layer with every shipped overlay loaded, as the window has.
...
fn every_shipped_overlay_reads() {
    ...
    // the region it belongs to answers no keys at all — which looks exactly like a panel that has
    // not been wired up.
    let (_window, _vim) = keys();
}

/// The keymap the git region ships with, parsed the way the window parses it.
fn git_keymap() -> zdt_vim::Keymap {
    let mut keymap = zdt_vim::Keymap::new();
    let shipped = zdt::assets::OVERLAYS
        .iter()
        .find(|(region, _, _)| *region == "git")
        .expect("the git region ships a keymap")
        .1;
    zdt_vim::config::merge(&mut keymap, shipped, zdt_vim::Leaders::default())
...
fn a_diff_flattens_to_one_row_per_line() {
    ...
    // say.
    use zdt::gitui::{DiffRow, diff_rows};
...
fn the_caret_walks_the_diff_a_line_at_a_time() {
    // Rows rather than hunks, so a long hunk can be read. What `s` stages is still a whole hunk.
