...
struct Inner {
    ...
    ///
    /// Held as the text rather than as a flag: a flag would have to be cleared on a timer, and a
    ...
    ///
    /// Taken once, because the write happens in a debounce timer and a context looked up there is
    /// not there — see `tests/context.rs`.
    notify: RefCell<Option<crate::notify::Notify>>,
}

/// How long after the last change the settings are written.
///
/// Long enough that a dragged slider is one write rather than sixty, short enough that letting go
/// and switching to another window finds the file already changed.
...
impl Settings {
    ...
    ///
    /// Set from the root once the announcements exist, because the settings are read before they
    /// are — the settings are what say whether announcements are wanted at all.
    ...
    ///
    /// The write is debounced, so a dragged slider is one write rather than one per pixel.
    ...
    pub fn persist_soon(&self) {
        let Some(timers) = self.inner.timers.clone() else {
            // No clock — a test, or a window that has gone. Write now rather than not at all.
            let _ = self.persist();
            return;
        };
    ...
    /// rather than read, so a second change to the same bytes from another window is applied.
    #[must_use]
