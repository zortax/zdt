...
fn BufferTab(
    ...
    view! {
        control(
            ...
            // is over the tab, and nothing otherwise. The slot is always there, so the name beside
            // it stays put when a buffer is edited.
            box(class = "tab__slot") {
                label(class = "tab__key") {{move || key().unwrap_or_default()}}
                // Drawn, and not typed. A bullet glyph sits wherever the font's metrics put it
                // inside its line box, which is off centre and different in two fonts. A box with
                // a radius is centred by geometry.
                box(class = "tab__mark") {}
                control(
                    class = "tab__close",
                    tabindex = Focus::Programmatic,
