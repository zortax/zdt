...
//! upstream was for. They are painted with the text, so they scroll with it and cost nothing per
//! frame.
...
pub fn follow(
    ...
    zgui::reactive::RenderEffect::new(move |_| {
        ...
        // The ranges are the server's, in its own units. They are converted against the text the
        // editor is holding, and never against what was sent.
        ...
        let (decorations, marks) = handle.query(|snapshot| {
            ...
            for one in &found {
                ...
                // A server can name an empty range, because a missing semicolon is at a point.
                // An underline of no width is one nobody can see. One character wide is the least
                // that says where.
...
/// Each part carries its own tone. Three errors and a hint are two different pieces of news, and a
/// status line that greys both says neither.
#[must_use]
